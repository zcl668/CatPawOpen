...
const _fetch = (url, options = {}) => {
  ...
  return axios({
    ...
    url: url,
    headers: headers,
    // axios 默认在响应中包含整个 response 对象，类似 fetch 的行为
    transformResponse: [(data) => data], // 保留原始响应，不自动解析 JSON
  }).then(response => ({
    ...
    if (error.response) {
      ...
      return {
        ...
        json: () => Promise.resolve(JSON.parse(error.response.data)),
      ...
      return {
        ...
        json: () => Promise.reject(new Error('No response received')),
      };
    } else {
      // 发送请求时出了点问题
...
export const initCloud = async (req) => {
  db = req.server.db
  localData = await db.getObjectDefault(`/tyi`, {})
}
